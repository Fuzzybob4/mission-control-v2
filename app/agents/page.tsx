      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        {view === "skills" ? (
          ...
          <div className={cn("flex gap-6", selected ? "items-start" : "")}>
            ...
            {/* Detail panel */}
            {selected && (
              <div ref={panelRef} className="w-80 flex-shrink-0 sticky top-24 space-y-4">
                    ...
                    {tasksLoading ? (
                      ...
                      <div className="px-4 py-6 text-center space-y-1">
                        ...
                        <p className="text-xs text-gray-600">
                          Create an <code className="text-gray-500">mc_tasks</code> table with columns:{" "}
                          <code className="text-gray-500">id, agent_id, title, status, created_at</code>
                        </p>
                      </div>
                    ) : tasks && tasks.length === 0 ? (
                      <div className="px-4 py-6 text-center text-xs text-gray-500">
                        No tasks logged for {selected.name}.
                      </div>
                    ) : tasks ? (
                      tasks.map(task => (
