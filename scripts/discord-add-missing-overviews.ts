...
const targets = [
  {
    ...
    topic: 'Business overview, positioning, systems, and operating notes for RedFox CRM.',
  },
  {
    category: 'KNIGHTFORGE | FROMINCEPTION',
    topic: 'Business overview, positioning, systems, and operating notes for FromInception.',
  },
] as const
...
async function main() {
  ...
  for (const target of targets) {
    const category = channels.find((c: any) => c.type === 4 && c.name === target.category)
    if (!category) continue
    let overview = channels.find((c: any) => c.type === 0 && c.parent_id === category.id && c.name === 'overview')
    if (!overview) {
      overview = await api('POST', `/guilds/${GUILD_ID}/channels`, headers, {
        name: 'overview',
        type: 0,
        parent_id: category.id,
        topic: target.topic,
      })
      console.log(`created overview: ${target.category}`)
    }
    const existing = await api('GET', `/channels/${overview.id}/messages?limit=5`, headers)
    if (Array.isArray(existing) && existing.length === 0) {
      await api('POST', `/channels/${overview.id}/messages`, headers, { content: template(target.category) })
      console.log(`seeded overview: ${target.category}`)
    }
  }

  console.log('Missing overviews added.')
}
