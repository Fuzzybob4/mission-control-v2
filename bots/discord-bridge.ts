...
const POLL_MS = 8000
const STATE_PATH = resolve(process.cwd(), 'memory/discord-bridge-state.json')

const WATCH_PARENT_CATEGORIES = new Set([
  'EXECUTIVE / STRATEGY',
  'AUTOMATION + AI (KAL ZONE)',
...
async function main() {
  ...
  while (true) {
    try {
      ...
      console.error('[discord-bridge] loop error:', err?.message || err)
    }
